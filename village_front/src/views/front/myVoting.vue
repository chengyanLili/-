      ...
      <div class="myMessages" v-for="(msg, index) in data.messageList" :key="index">
        <div class="msgBody">
          ...
          <div class="msgContent" style="display: block;">
              <ul class="info">
                  ...
                  <el-tag style="cursor: pointer;"  @click="handleDetail">点我报名</el-tag>
                  <el-tag style="margin-left:20px;cursor: pointer;" type="success" @click="candidate(msg.voteType)">查看候选人</el-tag>
              </ul>
          </div>
        </div>
      </div>
        <el-dialog width="60%" v-model="data.dialogTableVisible" title="报名选举">
          <el-form ref="formRef" :model="data.addForm" :rules="data.formRules">
            ...
            <el-form-item label="竞选职位" prop="voteType" :label-width="formLabelWidth">
            ...
            <el-form-item label="经历" prop="experience" :label-width="formLabelWidth">
              <el-input type="textarea" v-model="data.addForm.experience" placeholder="请输入主要职责" autocomplete="off" />
            </el-form-item>
            <el-form-item label="小组长" prop="whichGroup" :label-width="formLabelWidth">
              <el-input type="textarea" v-model="data.addForm.whichGroup" placeholder="如果竞选的是小组长，请输入具体的小组" autocomplete="off" />
            </el-form-item>
...
// 获取候选人
      request.get("/api/candidate/findPage", {
        params: {
          pageNum: 1,
          pageSize: 100,
          name:'',
...
const candidate = (voteType) => {
  ...
  router.push({path:'/front/candidateList',query: {
        voteType: voteType
        },}
)
}
