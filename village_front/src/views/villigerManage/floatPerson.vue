<template>
  ...
  > 
    ...
    <el-table-column prop="birthday" label="年龄" width="120" :formatter="formatAge"/>
    <el-table-column prop="idCard" label="身份证号码" width="168" />
    <el-table-column prop="whenCome" label="何时来此" width="120" :formatter="formatDates"/>
  ...
  <el-dialog 
    ...
    <el-form ref="formRef" :model="data.addForm" :rules="data.formRules">
      ...
      <el-form-item label="何时来此" prop="whenCome" :label-width="formLabelWidth">
        <el-date-picker
      ...
      <el-form-item label="暂住何户" prop="witchFamily" :label-width="formLabelWidth">
        <el-input v-model="data.addForm.witchFamily" placeholder="请输入现住何户" autocomplete="off" />
      </el-form-item>
      <el-form-item label="因何来此" prop="whyCome" :label-width="formLabelWidth">
...
const submit = () => {
  if(data.isEdit){
    ...
    if(valid){
      request.post("/api/floatPopulation/addOrUpdate",data.addForm).then((res)=>{
        if(res.status == 200){
          ElMessage({
            showClose: true,
            message: "修改成功！",
