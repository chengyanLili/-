<template>
  <div class="task">
    ...
    <div class="search">
      <el-input
        v-model="data.searchForm.searchName"
        clearable
        style="width: 150px; margin-right: 10px"
        placeholder="请输入标题"
      />
      <el-select
        v-model="data.searchForm.noticeType"
        clearable
        class="m-2"
    ...
    <el-button style="margin-bottom: 10px" type="primary" @click="handleAdd"
      >发布</el-button
    >
    <el-table :data="data.tableData" :border="true" :height="data.maxheight">
      ...
      <el-table-column prop="content" label="文章内容" width="120">
        <template #default="scope">
          <el-tag @click="view(scope.row.content)" type="warning"
            >查看内容</el-tag
          >
        </template>
      </el-table-column>
      ...
      <el-table-column label="任务状态" width="120">
        <template #default="scope">
          <el-tag v-if="scope.row.progress == '进行中'" type="success"
            >进行中</el-tag
          >
          <el-tag v-if="scope.row.progress == '已结束'" type="info"
            >已完成</el-tag
          >
        </template>
      </el-table-column>
      <el-table-column fixed="right" label="操作" width="250">
        <template #default="scope">
          ...
          <el-button size="small" type="danger" @click="handleDelete(scope.row)"
            >删除</el-button
          >
          <el-button type="info" size="small" @click="handleFinish(scope.row)"
            >结束任务</el-button
          >
        </template>
      </el-table-column>
      ...
      <el-form label-width="80px">
        <div class="input">
          ...
          <el-form-item label="请选择发布类型" label-width="200px">
            <el-select
              v-model="data.noticeForm.noticeType"
              clearable
              class="m-2"
              style="margin-right: 10px"
              placeholder="请选择通知类型"
        ...
        <el-form-item label="附件">
          ...
          >
            ...
            <template #tip>
              <div class="el-upload__tip" style="color: red; font-size: 14px">
                请上传 格式为 doc/xls/ppt/txt/pdf 的文件
              </div>
            </template>
          </el-upload>
        </el-form-item>
...
const save = () => {
  ...
  data.noticeForm.publishName = data.users[0].username;
  axios({
